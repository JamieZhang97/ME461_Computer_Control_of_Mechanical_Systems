...
__interrupt void cpu_timer2_isr(void)
{
    //the timer2 has been set to 1kHz, but we want to print to tera term and blink the LEDs at 10Hz. So we use a counter to realize that every 100 times enter this functiuon, we are ready to print and turn on/off the LEDs

	numTimer2calls++;
	if ((numTimer2calls % 100) == 0){
	    UARTPrint=1;
	    SetLEDColumnsOnOff(numTimer2calls/100);
	}

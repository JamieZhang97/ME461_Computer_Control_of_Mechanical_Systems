...
void ShowNumbyLED(int16_t switch_state);
...
__interrupt void cpu_timer2_isr(void)
{
    ...
    //First read switch state, and than use the switch state information as input to turn on/off LEDs to show figures
    switchdata=ReadSwitches();
    ShowNumbyLED(switchdata);
...
void ShowNumbyLED(int16_t switch_state){
    ...
    //print hex number by turn on/off LEDs
    //print 0
    if (switch_state == 0x0){
    ...
    //print B
        ...
        GpioDataRegs.GPACLEAR.bit.GPIO24 = 1; // LED16 Off
        GpioDataRegs.GPACLEAR.bit.GPIO25 = 1; // LED17 Off
        GpioDataRegs.GPASET.bit.GPIO26 = 1; // LED18 Off

...
//    init_serial(&SerialD,115200,serialRXD);
    ...
    {
        if (UARTPrint == 1 ) {
				serial_printf(&SerialA,"This is the number of times the timer2 called %d Number of Recieve bytes %ld\r\n", switchdata,numRXA);
            UARTPrint = 0;
        }
    }
}
...
__interrupt void cpu_timer2_isr(void)
{
	...
	if ((GpioDataRegs.GPDDAT.bit.GPIO123 | GpioDataRegs.GPDDAT.bit.GPIO124) == 1){
	    numTimer2calls++;
	}

...
//    init_serial(&SerialD,115200,serialRXD);
    ...
    {
        if (UARTPrint == 1 ) {
				serial_printf(&SerialA,"The switch state is %d Number of Recieve bytes %ld\r\n", switchdata,numRXA);
            UARTPrint = 0;
        }
    }
}
...
__interrupt void cpu_timer2_isr(void)
{
    //When enter this function UARTPrint change to 1, which means it is ready to print to Tera Term. And count the number of times that enter this function
	UARTPrint=1;
    numTimer2calls++;
...
//    //test turn on/off LEDS function by input a keep increasing number
//    testcount++;
//    SetLEDColumnsOnOff(testcount);
//
    //First read switch state, and than use the switch state information as input to turn on/off the columns of LEDs to show which switch is pressed
    switchdata=ReadSwitches();
    SetLEDColumnsOnOff(switchdata);
}

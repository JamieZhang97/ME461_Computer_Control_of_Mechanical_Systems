...
//    SpibRegs.SPITXBUF = 1200; //set initial transmit value¡¢
    ...
    {
        if (UARTPrint == 1 ) {
            serial_printf(&SerialA,"adc inputs are %f, %f \r\n", adcinput1, adcinput2);
            UARTPrint = 0;
        }
    }
}
...
__interrupt void cpu_timer0_isr(void)
{
	...
	//set a flag, to let sendvalue keep goes up/down, and bounce back at the limit
    if (updown == 1){
        sendvalue += 10;
        if (sendvalue >= 5200)
            updown = 0;
	...
	//right move the sendvalue to get the high 8 bits, and get the low 8 bits by &(0xFF) 
	...
	//SPI is MSB first, so left move to the high 8 bits
	SpibRegs.SPITXBUF = (uint16_t)(0xDA) << 8;
    SpibRegs.SPITXBUF = adc1in_MSB << 8; // RC1MSB
...
__interrupt void SPIB_isr(void){
	//receive a rubbish message
    spirec1 = SpibRegs.SPIRXBUF; // Read first 8 bit value off RX FIFO. Probably is zero since no chip

    adc1MSB = SpibRegs.SPIRXBUF; // Read second 8 bit value off RX FIFO. Again probably zero
    adc1LSB = SpibRegs.SPIRXBUF;
    adc2MSB = SpibRegs.SPIRXBUF;
    adc2LSB = SpibRegs.SPIRXBUF;

    GpioDataRegs.GPASET.bit.GPIO2 = 1; // Set GPIO 2 to end Slave Select. Now to Scope. Later to deselect DAN777
	//combine the high 8 bits and low 8 bits
    adc1 = (adc1MSB << 8) | (adc1LSB);
    adc2 = (adc2MSB << 8) | (adc2LSB);
	//scale to 0-3.0V
    adcinput1 = adc1*3.0/1024.0;
    adcinput2 = adc2*3.0/1024.0;

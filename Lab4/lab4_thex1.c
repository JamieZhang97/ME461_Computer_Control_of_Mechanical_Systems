...
//    GpioDataRegs.GPACLEAR.bit.GPIO11 = 1;
//
//    GPIO_SetupPinMux(160, GPIO_MUX_CPU1, 1);
...
//    EPwm6Regs.AQCTLB.bit.ZRO = 2;
    ...
    //initialize the PWM register 8A, 8B
    EPwm8Regs.TBCTL.bit.CTRMODE = 0;
    EPwm8Regs.TBCTL.bit.FREE_SOFT = 2;
    EPwm8Regs.TBCTL.bit.PHSEN = 0;
    //if we continuously set CLKDIV = 0, then TBPRD needs to be set as 1000000, which is larger than an uint_16 integer can represent, so set it as /16, then TBPZrD is 62500, satisfies the limit
    EPwm8Regs.TBCTL.bit.CLKDIV = 0x4;
    ...
    while(1)
    {
        if (UARTPrint == 1 ) {
            ...
            serial_printf(&SerialA," %f of Recieve bytes %ld\r\n", (3.3*(EPwm8Regs.CMPA.bit.CMPA)/(EPwm8Regs.TBPRD)),numRXA);
            UARTPrint = 0;
        }
    }
}
...
//    }
    //design a state machine, set four positions. In each case, the servo motor will turn to one positioin and change the state flag to the next, and when the next time the timer0 is called, turn to the next posotion.
    if(state == 1){
        EPwm8Regs.CMPA.bit.CMPA = 8000;
        state = 2;
    }
...
//            updown = 1;
    ...
    // create a state machine to generate the go up hill and down hill.
    // if the flag equals to 1, than increase CMPA to larger duty cycle. if CMPA goes up to the limit, change the flag to 0, so that next time it comes here, CMPA will decrease. Also if it touch the lower limit, the flag change to 1,so CMPA will increase.

//    if (updown == 1){
//        EPwm8Regs.CMPA.bit.CMPA +=50;
//        if (EPwm8Regs.CMPA.bit.CMPA >=  130000)
//            updown = 0;
//    }
//    else if (updown == 0){
//        EPwm8Regs.CMPA.bit.CMPA -=50 ;
//        if (EPwm8Regs.CMPA.bit.CMPA <= 30000)
//            updown = 1;
//
//    }
